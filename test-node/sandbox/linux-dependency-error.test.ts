...
// Node.js test runner does not support ESM module mocking (mock.module is a
// no-op in our compat layer). These tests rely on mocking
// generate-seccomp-filter.js exports, so they can only run reliably under Bun.
const skipReason =
  'Requires ESM module mocking (mock.module) — only available in Bun test runner'
...
void mock.module('dist/sandbox/generate-seccomp-filter.js', () => ({
...
const { checkLinuxDependencies, getLinuxDependencyStatus } =
  await import('../../dist/sandbox/linux-sandbox-utils.js')
...
describe('checkLinuxDependencies', { skip: skipReason }, () => {
...
describe('getLinuxDependencyStatus', { skip: skipReason }, () => {
  beforeEach(() => {
    mockBwrapInstalled = true
    mockSocatInstalled = true
    mockBpfPath = '/path/to/filter.bpf'
